...
BASE=http://127.0.0.1:3221 node --env-file=.env --import tsx scripts/verify-match-format.mts   # 89 checks on emulated iPhone 13 + Pixel 5: best of 3 / best of 5, the End set button and questions, one set rule courtside, in the parent view and at End match, and past matches unchanged
BASE=http://127.0.0.1:3223 node --env-file=.env --import tsx scripts/verify-undo.mts   # 94 checks on emulated iPhone 13 + Pixel 5: Set err and Dig err end the rally like Attack err; undo reverses the stat, the point, the serve and the rotation, in order, keeping hand changes; ended sets and finished matches ask first
